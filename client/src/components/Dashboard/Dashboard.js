...
class Dashboard extends Component {
    ...
    renderPosts(posts) {
        ...
        if (this.state.loading) {
            ...
            return(
                <p className="lead text-center"> {this.state.message} </p>
            )
            ...
            return (
                <div className="row container mx-auto">
                    {posts.map((post, index) => (
                        ...
                        <div className="card col-md-auto ml-5 mr-4 mb-4 offset-md-4" key={index}>
                            ...
                            <div className="mt-2">
                                ...
                                <img src={this.getPostImage(post._id)} alt="postImage" className="postImage" />
                            </div>
                            ...
                            <div className="postFooter mt-2">
                                <div style={{ display: 'flex' }}>
                                    ...
                                    {this.checkLiked(post) ?
                                                        (<img src={liked} className="postFooterIcons mr-2"
                                                         id="likeButton" onClick={()=>this.handleLikeUnlike(post._id)} alt=""
                                                     />
                                                        ):
                                                        (<img src={unliked} className="postFooterIcons mr-2"
                                                        id="likeButton" onClick={()=>this.handleLikeUnlike(post._id)} alt=""
                                                    />
                                                       )
                                    ...
                                    <img src={comment} className="postFooterIcons ml-2" alt="" onClick={()=>this.openSinglePostModal(post._id)}/>
                                </div>
                                <div className="searchResultUsername ml-1 mt-2">
                                    {post.likedBy? (<> {post.likedBy.length} Likes</> ):
                                    (<> 0 Likes </>)
                                } 
                                </div>
                                ...
                                <Link to={`/user/${post.postedBy._id}`}  style={{ textDecoration: "none" }}>
                                    ...
                                    <p className="postCaption ml-1 mt-2">
                                         {post.caption}
                                    </p>
                                </div>
                                {post.comments.length!==0?(
                                    ...
                                    {post.comments.length===1? (<p className="postCaption" onClick={()=>this.openSinglePostModal(post._id)} style={{cursor:"pointer"}}> View Comment </p>):
                                (<p className="postCaption"  onClick={()=>this.openSinglePostModal(post._id)}
                                style={{cursor:"pointer"}}> View all {post.comments.length} Comments </p>)}
                                </div>
    ...
    handleLikeUnlike(postId) {
        ...
        likeUnlikePost(userId, token, postId, type)
            .then(data => {
                if (data.error) {
                    ...
                    this.setState(prevState => ({
                        posts: prevState.posts.map(
                            singlePost => (singlePost._id === postId ? Object.assign(singlePost, { likedBy: data.post.likedby }) : singlePost)
                        )
                    }));
    ...
    returnModal(post){
       ...
       // console.log("return model called");
        if(this.state.singlePost===true && this.state.currentSinglePost===post._id){
            return(
                <SinglePostModal show={this.state.singlePost} modalClosed={this.closeModal} post={post}
                comments={post.comments} updateComment={this.updateComment}/>
            )
        }
    }
    ...
    updateComment(postId,newComment){
        ...
        this.state.posts.find(post=>{
            if(post._id===postId){
                ...
                this.setState({
                    posts: this.state.posts.map(post => (post._id === postId ? {...post,comments:newComment} : post))
                  });
