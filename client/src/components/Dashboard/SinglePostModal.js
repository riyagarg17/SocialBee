...
class SinglePostModal extends Component {
  ...
  handleSubmit(event) {
    ...
    addComment(user._id, token, post._id, comment).then(data => {
  ...
  handleDelete(comment) {
    ...
    const post = this.props.post;
    const token = isAuthenticated().token;
    deleteComment(user._id, token, post._id, comment).then(data => {
      //console.log("data recieved after submit",data.user);
      if (data.error) this.setState({ error: data.error });
      else {
        //console.log(data);
  ...
  render() {
    ...
    const singlePostPhoto = `${process.env.REACT_APP_API_URL}/postImage/${
      post._id
    }`;
          ...
          <div className="SinglePostContainer">
            ...
            <article style={{ width: "60%", height: "300px" }}>
              ...
              <div className="commentSection">
                <ul
                  style={{ padding: "0", marginBottom: "0", marginTop: "4px" }}
                >
                    ...
                    <li className="eachComment mb-2">
                      <div style={{ display: "flex" }}>
                        {comment.commentUser.image ? (
                          <img
                            alt="profile"
                            src={this.getPostUserImage(comment.commentUser._id)}
                            style={{
                              borderRadius: "50%",
                              width: "40px",
                              height: "40px"
                        ...
                        <div className="ml-3" style={{ display: "flex" }}>
                          <Link
                            to={`/user/${comment.commentUser._id}`}
                            style={{ textDecoration: "none" }}
                            key={i}
                          >
