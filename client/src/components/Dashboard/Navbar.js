...
class Navbar extends Component {
  ...
  async handleChange(event) {
    ...
    if (this.state.searchValue.length > 0) {
      ...
      searchUser(value, token).then(data => {
        ...
        if (data.error) {
          ...
          this.setState({ searchResults: data.users, display: true });
        }
      });
    }
      ...
      <>
        {isAuthenticated().user && (
          <nav
            ...
            style={{
              display:
                this.props.currRoute !== "/" &&
                this.props.currRoute !== "/login"
                  ? ""
                  : "none"
            }}
          >
            <a
              className="navbar-brand appName ml-5"
              href="/dashboard"
              style={{ fontSize: "42px", textDecoration: "none" }}
            >
              SocialBee
            </a>
            ...
            >
              ...
              <li className="dropdown">
                <Link
                  ...
                  to="#"
                >
                  <i className="fas fa-cog fa-2x" style={{ color: "black" }} />{" "}
                </Link>
                <div className="dropdown-menu ">
                  ...
                  <Link
                    className="dropdown-item"
                    to={`/settings/${isAuthenticated().user._id}`}
                    style={isActive(this.props.history, "/settings")}
                  >
                    <i className="fas fa-user-circle" /> Account Settings{" "}
                  </Link>
                </div>
              </li>
            </ul>
